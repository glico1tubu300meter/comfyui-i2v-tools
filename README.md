...
| アニメ調(入力画像) | 実写調(入力画像) | 実写調 → 動画生成(SVD-XT) |
| --- | --- | --- |
| ![アニメ調のサンプル画像](assets/sample_anime.png) | ![実写調のサンプル画像](assets/sample_beach.png) | ![生成された動画](assets/sample_beach_generated.gif) |

*入力画像はローカルのStable Diffusionで生成(実在の人物・作品とは無関係)。右は実写調の画像をSVD-XTで動画化した結果(波が静かに満ちる短いループ)。マルチキーフレーム誘導・マスク保護img2imgは、こうした画像を起点に長尺動画やバリエーション生成を行う。*
